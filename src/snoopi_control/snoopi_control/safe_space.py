...
# Uses LiDAR PointCloud2 for obstacle detection with bin-based safe angle finding.
# Odometry-based turning to the safest heading when obstacles are detected.
# Original authors: John Mann and Mihir Patel
# Adapted for Docker container deployment on RPi5.
#
# Usage (inside snoopi-ros2 container):
#   ros2 run snoopi_control safe_space
#
# Requires go2_driver_node to be running (provides /point_cloud2, /odom, cmd_vel topics).
...
class Go2Mover(Node):
    ...
    def move(self):
        ...
        while rclpy.ok():
            ...
            if state == FORWARD:
                if self.obs.obstacle_detected_left or self.obs.obstacle_detected_right:
                    state = STOPPED_SAFE
                    self.turn_end_time = None
                else:
                    msg.linear.x = 0.325
